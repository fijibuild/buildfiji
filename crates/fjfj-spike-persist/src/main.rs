...
struct Args {
    ...
    reps: u32,
    /// Replicate the loaded graph this many times (buildfiji-23d.19: confirm
    /// budgets hold at 1M+ nodes by replication rather than a real fixture
    /// that large). 1 = measure the loaded graph as-is.
    #[arg(long, default_value_t = 1)]
    replicate: usize,
    /// Skip redb and fjall, which are already known to be 7-8x larger and
    /// much slower to write; at 1M+ nodes their write time dominates the
    /// run for a result this spike already has.
...
fn main() -> Result<()> {
    ...
    let g = if args.replicate > 1 {
        synth::replicate(&g, args.replicate)
    } else {
        g
    };
